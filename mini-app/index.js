...
app.post('/books', upload.single('image'), async (req, res) => {
  try {
    ...
    const imageUrl = req.file ? `http://192.168.194.10:3000/uploads/${req.file.filename}` : '';
...
app.put('/books/:id', upload.single('image'), async (req, res) => {
  try {
    ...
    if (req.file) {
      updateData.imageUrl = `http://192.168.194.10:3000/uploads/${req.file.filename}`;
    }
...
app.listen(PORT, '0.0.0.0', () => console.log(`Server running at http://192.168.194.10:${PORT}`));
